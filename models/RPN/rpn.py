...
class RPN(nn.Module):
    def __init__(self, din):
        ...
        self.feat_stride = rpn_cfg.FEAT_STRIDE[0]
...
if __name__ == '__main__':
    rpn = RPN(512)
    rpn.training = False
    x = torch.Tensor(1, 512, 32, 32)
    im_info = torch.FloatTensor(1, 2)
    num_boxes = torch.LongTensor(1, 1)
    gt_boxes = torch.FloatTensor(1, 1)
    out, _, _ = rpn(x, im_info, gt_boxes, num_boxes)
    print(out.shape)